...
class Survival(object):
	...
	def get_input_data(self):
		...
		print('Data available:\t{}\n'.format(self.training_data.columns.values.tolist()))
		predict_data_pd = self.predict_data[['Pclass', 'Sex', 'Age', 'Fare', 'PassengerId']]
		...
		for raw_data in predict_data_nottransformed:
			...
			data_passengers[raw_data[4]] = line
			predict_data.append(line)

		#~ print(data_passengers)
...
if __name__ == '__main__':
	...
	predictions = []
	with open('submission_file.csv', 'w') as report:
		report.write('PassengerId,Survived\n')
	for passenger in predict_data:
		statut = survival.predict(row=predict_data[passenger], weights=weights)
		with open('submission_file.csv', 'a') as report:
			report.write('{},{}\n'.format(passenger, statut))

		passenger_translated = []
		passenger_translated.append(predict_data[passenger][0])
		passenger_translated.append('Men' if predict_data[passenger][1] == 1 else 'Women')
		passenger_translated.append(predict_data[passenger][2])
		passenger_translated.append(predict_data[passenger][3])
